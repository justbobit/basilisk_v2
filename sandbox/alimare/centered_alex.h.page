...
face vector uf[];
/**
In the specific case where we have two phases calculated we store the values in p2,u2... fields. At the end of each iteration we swap the variables and then the solver can be reapplied to the new set of variables.  

TBD: one has to be careful to have all phases synchronized. Each phase is still run with its own timestep, therefore there is no coupling between both phases. 
*/
#if DOUBLE_EMBED
...
#if DOUBLE_EMBED
  uf2.x.refine = refine_face;
  foreach_dimension(){
    uf2.x.prolongation = refine_embed_face_x;
  }
  for (scalar s in {p2, pf2, u2, g2}) {
    s.restriction = restriction_embed_linear;
    s.refine = s.prolongation = refine_embed_linear;
  }
  for (scalar s in {p2, pf2})
    s.embed_gradient = pressure_embed_gradient;
#endif // DOUBLE_EMBED
#endif // EMBED
#endif // TREE
...
event init (i = 0)
{
  ...
  boundary ((scalar *){u2});
  restriction((scalar *) {u2});
  trash ({uf2});
  foreach_face(){
    uf2.x[]     = (1-fm.x[])*face_value (u2.x, 0);
  }
  boundary({p2,pf2});
  boundary((scalar *){g2,uf2});
  restriction({p2,pf2});
  restriction((scalar *) {g2,uf2});
...
#if DOUBLE_EMBED
event set_dtmax (i+=2,last) dtmax = DT;
event stability (i+=2,last) {
  double dt1 = dtnext (stokes ? dtmax : timestep (uf, dtmax));
  foreach()
      cs[]      = 1.-cs[];
  ...
  double tnext1 = tnext;
  double dt2 = dtnext (stokes ? dtmax : timestep (uf2, dtmax));
...
/**
Double calculation inside and outside of the boundary must be done before LS
advection else theres is an inconsistency on the boundary definition. In theory, only the pointers should be swapped and not all the values of all the fields.
*/
