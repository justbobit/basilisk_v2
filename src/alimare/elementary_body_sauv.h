...
void phase_change_velocity (scalar f, scalar tr, face vector v_pc,\
                            double D_L, double D_S, double peclet_S,\
                            double peclet_L) {
  scalar D = tr.D , peclet = tr.peclet ;
  foreach(){
    f[] = clamp(f[], 0., 1.);
    D[] = f[]* D_L + (1.-f[])*D_S;
    peclet[] = f[]* peclet_L + (1.-f[])*peclet_S;
  }
    
  boundary ({f, tr, D, peclet});
...
mgstats dirichlet_diffusion (struct Dirichlet_Diffusion p) {
  ...
  foreach() {
    volumic_metric[] = cm[];
    if (p.tr_op.i)  
      dirichlet_source_term[] = cm[]*tr_eq[]*tr_op[]*D[]*time_factor*(tr.inverse ?
                                  f[] : 1. - f[])*sq((1<<max_level)/L0);
    else
      dirichlet_source_term[] = cm[]*tr_eq[]*D[]*time_factor*(tr.inverse ?
                                  f[] : 1. - f[])*sq((1<<max_level)/L0);
    dirichlet_feedback_term[] = - cm[]*D[]*time_factor*(tr.inverse ?
                                  f[] : 1. - f[])*sq((1<<max_level)/L0);
  }
  foreach_face()
    diffusion_coef.x[] = fm.x[]*D[];
