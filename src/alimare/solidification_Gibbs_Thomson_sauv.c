...
#define T_END   16.
#define DT_MAX  0.2
#define DELTA_T 0.2 // for videos and measurements
...
#define SIGMA                 0.007
#define Ray_min               L0
#define Precoeff              6.*(T_eq-TS_inf)/(SIGMA*Ray_min)
...
double hexagon (double x, double y, double h)
{
  ...
  double kk = tan(Pi/12.);
  return (theta < Pi/(12.) ? x-h : (theta < 5.*Pi/(12.) ? y+x-h*(kk+1.) : y-h));
}
...
event stability (i++) {
  ...
  if (i == 0)
    ...
    for (scalar t in tracers) {    
      face vector tv[];
      phase_change_velocity (f, t, tv, D_L, D_S, peclet_S, peclet_L);
      foreach_face()
...
event movie (t = 0.; t += max(DELTA_T, DT); t <= T_END)
{
  ...
  output_ppm (temperature, n = 512, linear = true, file = "T.gif", opt = "--delay 1",min = -1, max = 1);
